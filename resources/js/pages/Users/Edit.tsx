            ...
            <form onSubmit={submit} className="space-y-6 rounded-xl border bg-card p-6 shadow-sm">
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <div className="space-y-2">
                        ...
                        {errors.role_id && <div className="text-sm text-red-500">{errors.role_id}</div>}
                    </div>
                    <div className="space-y-2"><Label htmlFor="site_id">Site *</Label>{mounted && (<Select value={data.site_id} onValueChange={(val) => setData('site_id', val)}><SelectTrigger><SelectValue placeholder="Select site" /></SelectTrigger><SelectContent>{sites.map((site) => (<SelectItem key={site.id} value={site.id.toString()}>{site.name}</SelectItem>))}</SelectContent></Select>)}{errors.site_id && <div className="text-sm text-red-500">{errors.site_id}</div>}</div>
                </div>
                <div className="flex justify-end pt-4"><Button type="submit" disabled={processing}>Update User</Button></div>
            </form>
        </div>
    );
}

UserEdit.layout = { breadcrumbs: [{ title: 'Edit User', href: '#' }] };
