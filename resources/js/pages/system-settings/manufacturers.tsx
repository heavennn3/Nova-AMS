...
export default function Manufacturers({ data = [] }: ManufacturersProps) {
    ...
    const handleDelete = (id: number) => {
        ...
        router.delete(`/settings/manufacturers/${id}`, {
            preserveScroll: true,
        });
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const url = editingItem
            ? `/settings/manufacturers/${editingItem.id}`
    ...
    const columns = [
        ...
        },
        {
            ...
            header: ({ column }: any) => (
                <DataTableColumnHeader column={column} title="Support Email" />
            ),
            cell: ({ row }: any) => row.original.support_email || <span className="text-muted-foreground">-</span>,
        },
        {
            accessorKey: 'support_phone',
            header: ({ column }: any) => (
                <DataTableColumnHeader column={column} title="Support Phone" />
            ),
            cell: ({ row }: any) => row.original.support_phone || <span className="text-muted-foreground">-</span>,
        },
        {
            id: 'actions',
            header: 'Actions',
    ...
    return (
        <div className="w-full space-y-8 p-8">
            <Head title="Manufacturers Settings" />
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                        Manage manufacturer info for tracking asset origins & support contacts.
                    </p>
                </div>
            </div>
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>
                            {editingItem ? 'Edit' : 'Add New'} Manufacturer
                        </DialogTitle>
                    </DialogHeader>
